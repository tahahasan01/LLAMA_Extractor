...
# ── 4.  Folder layout ─────────────────────────────────────────────────-
ROOT      = Path.cwd()          # wherever you run python batch_llamaparse.py
DATA_DIR  = ROOT / "Data"       # PDFs live here
...
# ── 4a. Clean all existing output folders ─────────────────────────────
...
if not pdfs:
    raise SystemExit(f"No PDFs found in {DATA_DIR.resolve()}")
...
for pdf_path in pdfs:
    name = pdf_path.stem
    print(f"\n=== Parsing {pdf_path.relative_to(DATA_DIR)} ===")

    # -- 6a. run the job and grab JSON ----------------------------------
    json_objs = parser.get_json_result(str(pdf_path))  # list; one element for 1 file
    pages     = json_objs[0]["pages"]
    
    # Add to combined JSON data
    all_json_data.extend(json_objs)

    # -- 6b. extract images (using the WORKING approach) ---------------
    raw_img_folder = OUT_ROOT / "temp_images"
    raw_img_folder.mkdir(parents=True, exist_ok=True)
    
    # Extract raw images to temporary location
    _ = parser.get_images(json_objs, download_path=str(raw_img_folder))
    ...
    # -- 6c. save text and markdown per page ----------------------------
    for p in pages:
...
# -- 7. Save combined JSON data ----------------------------------------
(DIR_JSON / "all_data.json").write_text(json.dumps(all_json_data, indent=2))

print("\n✅  Done.  Check the 'parsed/' folder for organized outputs:")
